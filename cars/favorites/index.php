...
$APPLICATION->SetTitle("Избранные автомобили");
?>
<?php
define('ENTITY', 'CIS_FAVORITES');
...
if ( isset($_GET['debug']) && $_GET['debug'] == 1 ) {
    YAppShowroom::sp($_COOKIE, false, 'COOKIES');
    YAppShowroom::sp($items, false, 'ITEMS');
    if (!empty($api_url)) YAppShowroom::sp($api_url, false, 'API URL');
    YAppShowroom::sp($data, false, 'API DATA');
}

$GLOBALS['META'] = $data['meta'] = json_decode( YAppShowroom::httpGet('https://apps.yug-avto.ru/API/get/cis/meta/new/?token=34b5ac8b71018c0bc7e5c050ed90b243&site=yug-avto.ru&entity=new&brand=favorites'), true );

if ( !empty($data) && is_array($data) ) {
    $data['FAVORITES'] = $items;
    $data['COMPARE'] = ( !empty($_COOKIE['CIS_COMPARE']) && is_string($_COOKIE['CIS_COMPARE']) ) ? json_decode($_COOKIE['CIS_COMPARE'], true) : [];
    if ( !is_array($data['COMPARE']) ) $data['COMPARE'] = [];
}
?>
