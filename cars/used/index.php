...
<?php if ( $data['meta']['meta']['level'] == 'vehicle' ) { 
    ...
    $cleanTitle = trim('С пробегом ' . $brandName . ' ' . $modelName);
    if ($prodYear) {
    ...
    $schemaData = [
        ...
        "@type" => "Product",
        "name" => $cleanTitle ?: htmlspecialchars_decode($data['meta']['meta']['title'] ?? ''),
...
<?php } else { 
    ...
    if (!empty($data['items']) && is_array($data['items'])) {
        ...
        foreach (array_slice($data['items'], 0, 24) as $item) {
            ...
            $itemUrl = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'] . $app->Conf()['assetsUrl'] . '/' . ($item['entity'] ?? 'used') . '/' . ($item['brand']['code'] ?? '') . '/' . ($item['model']['code'] ?? '') . '/' . ($item['id'] ?? '') . '/';

            $offerList[] = [
                "@type" => "Offer",
                "name" => $itemName ?: "Автомобиль с пробегом Юг-Авто",
                "price" => $itemPrice,
                "priceCurrency" => "RUB",
                "url" => $itemUrl,
                ...
                "seller" => [
                    "@type" => "AutoDealer",
                    "name" => !empty($item['dealership']['name']) ? "Юг-Авто Эксперт (" . $item['dealership']['name'] . ")" : "Юг-Авто Эксперт",
                    "url" => $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST']
                ],
                "image" => $itemImg ?: null
            ];
    ...
    $listSchema = [
        "@context" => "https://schema.org/",
        "@type" => "Product",
        "name" => htmlspecialchars_decode($data['meta']['meta']['title'] ?? 'Купить автомобили с пробегом в Юг-Авто'),
