...
<div class="pb-5 bg-yalightbluegray">
	<div class="container" data-form="<?= $arResult['arForm']['SID'];?>">
		<div class="row">
			<div class="col">
				<div class="bg-yawhite b-radius-yaradius-16 p-4">
					<div class="row">
						<div class="col-lg-8">
							<div class="row h-100">
								<div class="col-lg-7 d-flex align-items-center mb-4 mb-lg-0 pe-4">
									<img class="b-radius-yaradius-16 overflow-hidden w-100" src="<?= $arParams['PARAM_IMAGE'];?>" />
								</div>
								<div class="col-lg-5 d-flex flex-column justify-content-between align-items-start px-4 px-lg-0">
									<img class="w-100 mb-4 mb-lg-0" src="<?= CFile::GetPath($arResult['SETTINGS']['PROPERTY_TITLE_IMAGE_VALUE']);?>" />
									<div class="form-description c-yadarkgray"><?= $arResult['FORM_DESCRIPTION'];?></div>
								</div>
							</div>
						</div>
						<div class="col-lg-4">
							<div class="b-radius-yaradius25 bg-yawhite p-2 h-100 d-flex justify-content-center card-v-flex">
								<form class="row text-minus position-relative z-index-1" data-sid="<?= $arResult['arForm']['SID'];?>">
									...
									<div class="col-12 mb-3">
										<div class="form-group">
											<div class="form-input-title bg-yalightbluegray d-flex flex-column justify-content-between align-items-start">
												<div class="title fw-bold text-minus"><?= $arParams['PARAM_VEHICLE'];?></div>
												<div class="price d-flex justify-content-between align-items-center w-100">
													<div class="fw-bold text-plus"><?= number_format($arParams['PARAM_MIN_PRICE'], 0, '.', ' ');?> ₽</div>
													<?php if ($arParams['PARAM_PRICE']) { ?>
													<div class="text-minus-minus text-decoration-line-through c-yadarkgray"><?= number_format($arParams['PARAM_PRICE'], 0, '.', ' ');?> ₽</div>
													<?php } ?>
												</div>
											</div>
										</div>
									...
									<?php foreach ( $arResult['arQuestions'] as $arItem ) { ?>
										<?php switch ( $arItem['SID'] ) {
											...
											case 'VEHICLE':
												?>
												<input type="hidden" name="<?= $arItem['SID'];?>"  value="<?= $arParams['PARAM_VEHICLE'];?>" />
												<?php 
												break;
											case 'VIN':
												?>
												<input type="hidden" name="<?= $arItem['SID'];?>" value="<?= $arParams['PARAM_VIN'];?>" />
												<?php 
												break;
