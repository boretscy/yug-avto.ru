...
<?php } else { ?>
	<div class="container my-4" itemscope itemtype="http://schema.org/Blog">
		<meta itemprop="description" content="Статьи и блог Юг-Авто — полезные материалы и обзоры автомобилей в Краснодаре">
		<div class="row">
			<?php foreach ( $arResult['ITEMS'] as $arItem ) { ?>
			<?php
				$arItem['__NAME'] = !empty($arItem['NAME']) ? str_replace("Юг-Авто", "<span style='white-space: nowrap;'>Юг-Авто</span>", $arItem['NAME']) : '';
				$detailUrl = !empty($arItem['DETAIL_PAGE_URL']) ? ((strpos($arItem['DETAIL_PAGE_URL'], 'http') === 0) ? $arItem['DETAIL_PAGE_URL'] : $host . $arItem['DETAIL_PAGE_URL']) : $host;
				...
				$previewText = !empty($arItem['PREVIEW_TEXT']) ? strip_tags($arItem['PREVIEW_TEXT']) : htmlspecialchars((string)($arItem['NAME'] ?? ''));
			?>
			<div class="col-md-6 col-xl-4 mb-4 blog-item" itemprop="blogPosts" itemscope itemtype="http://schema.org/BlogPosting">
				<div class="bg-yalightbluegray b-radius-yaradius-16 overflow-hidden text-start w-100">
					...
					<div class="p-4">
						...
						<a 
							...
							class="c-yablack c-h-yablack text-decoration-none blog-item-title fw-bold d-flex justify-content-between align-items-start block-title-link"
							>
							<h2 itemprop="headline" class="title h5 m-0 fw-bold"><?= $arItem['__NAME'];?></h2>
